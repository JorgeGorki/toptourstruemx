            ...
            if ($post_type == 'st_hotel' or $post_type == 'hotel_room') {
                $html = '<div style="text-align: center; padding: 10px 0px; font-weight: bold; border-left: solid 1px #666; border-right: solid 1px #666; border-bottom: solid 1px #666;">
                        ...
                        <span style="text-align: right; width: 50%; display: inline-block;">
                             ' . $number . esc_html__('item(s)', ST_TEXTDOMAIN) . '
                        </span>
                        </div>
                ';
                return $html;
    ...
    function st_email_booking_sale_price()
    {
        ...
        if ($order_id) {
            ...
            if(isset($data_price['has_return'])){
                $carstranfer_html = '';
                $carstranfer_html .= esc_html__('Depart: ', ST_TEXTDOMAIN) . TravelHelper::format_money_from_db($data_price['price'], $currency);
                $carstranfer_html .= '<br>' .esc_html__('Return: ', ST_TEXTDOMAIN) . TravelHelper::format_money_from_db($data_price['price_return'], $currency);
    ...
    function st_email_booking_status()
    {
        ...
        if ($order_id) {
            ...
            switch ($html) {
                case 'incomplete':
                default:
                    $html = esc_html__('incomplete', ST_TEXTDOMAIN);
                    break;
                case 'complete':
                    $html = esc_html__('completed', ST_TEXTDOMAIN);
                    break;
                case 'canceled':
                    $html = esc_html__('canceled', ST_TEXTDOMAIN);
                    break;
                case 'pending':
                    $html = esc_html__('pending', ST_TEXTDOMAIN);
                    break;
            }
