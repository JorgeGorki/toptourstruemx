...
<h1 class="wp-heading-inline"><?php echo esc_html(__('Extensions','traveler')); ?></h1>
    <nav class="nav-tab-wrapper wpm-nav-tab-wrapper st-tab-extendsion">
        <a href="<?php echo admin_url().'admin.php?page=st-vina-extensions&extension-tab=list'; ?>" class="nav-tab <?php if(!isset($_GET['tab'])){echo 'nav-tab-active';}?>"><?php echo esc_html('Plugins',ST_TEXTDOMAIN)?></a>
        <a href="<?php echo admin_url().'admin.php?page=st-vina-extensions&extension-tab=extendsion'; ?>" class="nav-tab <?php if(isset($_GET['tab']) && $_GET['tab']==='extendsion'){echo 'nav-tab-active';}?>"><?php echo esc_html('Extension',ST_TEXTDOMAIN)?></a>
    </nav>
    <?php
    ...
    if(!$check_purchase){
        ...
        <div class="error" style="padding: 8px 10px 5px 10px !important; margin-left: 0px; margin-top: 15px;">
            <p class="about-description"><?php printf(__("Please register product before install demo content %s",ST_TEXTDOMAIN),'<a href="'. admin_url('<?php echo get_admin_url()?>admin.php?page=st_product_reg') .'" target="_blank">Click here.</a>')?></p>
        </div>
        <?php
        return;
    }
