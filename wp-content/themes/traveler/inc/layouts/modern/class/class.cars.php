    ...
    class STCars extends TravelerObject
    {
            ...
            add_filter( 'pre_get_posts', [ $this, 'removeSearchServiceLocationByAuthor' ] );
                ...
                <span class="count-string">
                    ...
                    if ($query_service->found_posts):
                        ...
                        echo sprintf(__('%d - %d of %d ', ST_TEXTDOMAIN), (int)$posts_per_page * ($page - 1) + 1, $last, $query_service->found_posts );
                        echo ( $query_service->found_posts == 1 ) ? __( 'Car', ST_TEXTDOMAIN ) : __( 'Cars', ST_TEXTDOMAIN );
                    endif;
                    ?>
                </span>
                <?php
        ...
        function do_add_to_cart()
        {
            ...
            if ( empty($_REQUEST['location_id']) ) {
                STTemplate::set_message(__('Location is required', ST_TEXTDOMAIN), 'danger');
                $pass_validate = FALSE;
                return false;
            }
            ...
            //     STTemplate::set_message(__('The Pick Up Date is required', ST_TEXTDOMAIN), 'danger');
            //     $pass_validate = FALSE;
            //     return false;
            // }
            if ( empty($_REQUEST['drop-off-date']) ) {
                STTemplate::set_message(__('The Drop Off Date is required', ST_TEXTDOMAIN), 'danger');
                $pass_validate = FALSE;
                return false;
            }
            $check_in = '';
            ...
            if (isset($_REQUEST['location_id']) && !empty($_REQUEST['location_id'])) {
                ...
                if (!empty($location_without_s)) {
                    if (!in_array($location_id, $location_without_s)) {
                        STTemplate::set_message(__('The location is not match with car location.', ST_TEXTDOMAIN), 'danger');
                        $pass_validate = false;
                        return false;
                    }
                }
                if (!$pickup_country) {
                    STTemplate::set_message(__('The \'country\' field not set for the \'' . get_the_title($location_id) . '\'', ST_TEXTDOMAIN), 'danger');
                    $pass_validate = false;
                    return false;
                }
