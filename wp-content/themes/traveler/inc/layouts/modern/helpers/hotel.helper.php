        ...
        class HotelHelper
        {
            ...
            public function _format_real_price()
            {
                ...
                if ( $price_by_per_person == 'on' ) {
                    $alert_numday = sprintf( __( 'per %d Person per %d Night(s)', ST_TEXTDOMAIN ), $total_person, 1 );
                    if ( $numberday > 0 ) {
                            $alert_numday = sprintf( __( 'per %d Person per %d Night(s)', ST_TEXTDOMAIN ), $total_person, $numberday );
                    }
                } else {
            ...
            public function _get_availability_hotel_room()
            {
                ...
                if ( !empty( $room_available ) ) {
                    foreach ( $room_available as $kk => $vv ) {
                        if($price_by_per_person === 'on'){
                            $ev = __('Adult: ', ST_TEXTDOMAIN) . TravelHelper::format_money($vv['adult_price']) . '<br/>';
                            $ev .= __('Child: ', ST_TEXTDOMAIN) . TravelHelper::format_money($vv['child_price']) . '<br/>';
