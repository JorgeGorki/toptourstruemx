...
<div id="<?php echo esc_attr($div_id);?>" class='<?php if (apply_filters('st_woo_cart_is_collapse' , false)) {echo esc_attr("collapse");}?>'>
    ...
    <div class="cart_item_group" style='margin-bottom: 10px'>
        ...
        <?php echo esc_html__('Room', ST_TEXTDOMAIN); ?>: <a style="color: inherit" href="<?php echo esc_attr($room_link) ; ?>"><b class='booking-cart-item-title'><?php echo get_the_title($st_booking_data['room_id']); ?></b></a>
        <ul class="booking-item-description">
            ...
            if ( $price_by_per_person == 'on' ) : ?>
                <li><?php echo __('Number of rooms:',ST_TEXTDOMAIN);?> <?php echo ($st_booking_data['room_num_search']); ?></li>
                <li><?php echo __('Adult Price:',ST_TEXTDOMAIN);?> <?php echo TravelHelper::format_money(isset($st_booking_data['adult_price']) ? $st_booking_data['adult_price'] : 0) ; ?></li>
                <li><?php echo __('Child Price:',ST_TEXTDOMAIN);?> <?php echo TravelHelper::format_money(isset($st_booking_data['child_price']) ? $st_booking_data['child_price'] : 0) ; ?></li>
                <?php
            else: ?>
                <li><?php echo __('Number of rooms:',ST_TEXTDOMAIN);?> <?php echo ($st_booking_data['room_num_search']); ?> x <?php echo TravelHelper::format_money($st_booking_data['item_price']) ; ?></li>
                <?php
            endif; ?>
        </ul>
    </div>
