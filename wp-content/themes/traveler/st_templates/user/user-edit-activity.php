...
<div class="st-create-service">
    <h2 class="st-header">
        <?php
        if(!empty($post_id)){
            echo __('Edit Activity', ST_TEXTDOMAIN);
        }else {
            echo __('Add Activity', ST_TEXTDOMAIN);
        }
        ?>
    </h2>
    ...
    <div class="st-create-service-content <?php echo (empty($post_id) ? ' add' : 'edit')?>">
        <div>
            ...
            <div class="st-partner-action">
                ...
                <button type="submit" class="form-control btn btn-primary st-btn-continue" data-obj="button" data-status="<?php echo esc_attr($data_status); ?>"><span><?php echo __('CONTINUE', ST_TEXTDOMAIN); ?></span> <i class="fa fa-spin fa-spinner"></i></button>
            </div>
        </div>
    </div>
</div>
