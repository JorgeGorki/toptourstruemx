...
if(!empty($list_message)){
    ...
    <div class="st-inbox-body">
        <h4 class="title"><?php echo sprintf(_n('You have %s message', 'You have %s messages',esc_html($count_item),ST_TEXTDOMAIN), '<span class="count_message">'.esc_html($count_item).'</span>') ?></h4>
        <div class="inbox-list-message">
            ...
            foreach($list_message as $key => $val){
                ...
                if(isset($obj->labels->singular_name)){
                    ...
                    echo '<a data-original-title="'.__('Remove', ST_TEXTDOMAIN).'" class="btn_remove_message btn-loading cursor fa fa-times" rel="tooltip" data-placement="top" data-message-id="'.esc_attr($val['id']).'"></a>';
