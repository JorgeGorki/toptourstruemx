...
<table class="table table-bordered table-striped table-booking-history">
    ...
    <?php
        ...
        while( have_posts() ): the_post();
            ...
            $action_cancel = '';
            if( (float)$post->cancel_refund > 0 && $post->cancel_refund_status == 'pending' ){
                $action_cancel = '<a data-toggle="modal" data-target="#with-refund-modal" href="javascript: void(0);" class="with_a_refund btn btn-primary btn-xs" data-order_id="'. esc_attr($post->order_item_id) .'" data-order_encrypt="'. TravelHelper::st_encrypt( $post->order_item_id ) .'">'. __('with refund', ST_TEXTDOMAIN) .'</a>';
            }
