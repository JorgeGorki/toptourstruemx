...
<div class="form-group form-extra-field dropdown clearfix field-guest-new  <?php if ( $has_icon ) echo ' has-icon '; ?>">
    ...
    <div class="dropdown " data-toggle="dropdown" id="dropdown-1">
        <label class="<?php echo ($class_hide_render == 'hide') ? '' : 'hide' ?>"><?php echo esc_html__( 'Guests', ST_TEXTDOMAIN ); ?></label>
        <div class="render <?php echo esc_attr($class_hide_render); ?>">
            <span class="adult" data-text="<?php echo esc_html__( 'Adult', ST_TEXTDOMAIN ); ?>"
                  data-text-multi="<?php echo esc_html__( 'Adults', ST_TEXTDOMAIN ); ?>"><?php echo sprintf( _n( '%s Adult', '%s Adults', esc_attr($adult_number), ST_TEXTDOMAIN ), esc_attr($adult_number) ) ?></span>
            -
            <span class="children" data-text="<?php echo esc_html__( 'Child', ST_TEXTDOMAIN ); ?>"
                  data-text-multi="<?php echo esc_html__( 'Children', ST_TEXTDOMAIN ); ?>"><?php echo sprintf( _n( '%s Child', '%s Children', esc_attr($child_number), ST_TEXTDOMAIN ), esc_attr($child_number) ); ?></span>
        </div>
    </div>
